...
CREATE OR REPLACE FUNCTION get_surface_km2 (geom GEOMETRY)
    RETURNS double precision
    AS $$
BEGIN
    RETURN ST_Area (ST_Transform (geom, 3857)) / 1e6;
END;
$$
LANGUAGE plpgsql;
...
SET
    ...
    WHERE
        id IN (
            ...
            WHERE
                core_georegion.insee_code IN ( SELECT DISTINCT
                        insee_reg
                    FROM
                        regions));

INSERT INTO "public".core_geozone (uuid, created_at, updated_at, deleted, geo_zone_type, geometry, "name", name_normalized)
SELECT
...
INSERT INTO "public".core_georegion (geozone_ptr_id, insee_code, surface_km2)
SELECT
...
SET
    ...
    WHERE
        id IN (
            ...
            WHERE
                core_geodepartment.insee_code IN ( SELECT DISTINCT
                        insee_dep
                    FROM
                        departments));
...
INSERT INTO "public".core_geodepartment (geozone_ptr_id, insee_code, region_id, surface_km2)
SELECT
    core_geozone.id AS geozone_ptr_id,
    ...
    (
        ...
        FROM
            core_georegion
        WHERE
            insee_code = departments.insee_reg) AS region_id,
    get_surface_km2 (core_geozone.geometry) AS surface_km2
FROM
    "public".core_geozone
...
SET
    ...
    WHERE
        id IN (
            ...
            WHERE
                core_geocommune.iso_code IN ( SELECT DISTINCT
                        insee_com
                    FROM
                        communes));
...
INSERT INTO "public".core_geocommune (department_id, geozone_ptr_id, iso_code)
SELECT
    (
        SELECT
            geozone_ptr_id
        FROM
            core_geodepartment
        WHERE
            insee_code = communes.insee_dep) AS department_id,
    core_geozone.id AS geozone_ptr_id,
